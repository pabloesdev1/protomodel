...
@cli.command()
@click.option('--python_file',
              help='Python file name with directory.')
@click.option('--proto_name',
              help='Proto buffer file name with directory.')
@click.option('--package_name',
              default=None,
              help='Pakage name to add in proto buffer file.')
def generate(python_file, proto_name, package_name):
    ...
    data_spec = importlib.util.spec_from_file_location(
        input_filename.split(".")[0], input_filename)
    ...
    messages: List[message] = [
        cls for cls in data_module.__dict__.values() if isinstance(cls, message)]
    services: List[service] = [
        cls for cls in data_module.__dict__.values() if isinstance(cls, service)]

    for msg in messages:
        msg.add_field(proto_file)

    for service_ in services:
        service_.add_field(proto_file)
