...
class Service:
    ...
    def add_field(self, file: TextIO):
        file.write(
            f"{self.__class__.__name__.lower()} {self.service_model.__name__} {{\n")
        rpc_list: list[type[rpc]] = [o for m, o in inspect.getmembers(
            self.service_model) if isinstance(o, RPC)]
        ...
        for rpc_method in rpc_list:
            ...
            params_list = []
            return_type = None
            rpc_annotations: dict[str,
                                  Any] = rpc_method.rpc_model.__annotations__
            ...
            file.write(
                f"  {rpc_method.__class__.__name__.lower()} {rpc_method_name} ({params_list}) returns ({return_type}) {{}}\n")  # noqa: E501
        file.write(f"}}\n\n")  # noqa: F541
